...
RM = rm -f
BIN = .
DEFINE = -D_GNU_SOURCE -DUSE_GCC_X64 -DPOPCOUNT
EXE = hqperft
...
ifeq ($(CC),clang)
	...
	PGO_USE = -fprofile-instr-use=hqperft.profdata
	PGO_MERGE = llvm-profdata merge -output=hqperft.profdata hqperft-*.profraw
...
#commands
all :
	$(CC) $(CFLAGS) hqperft.c -o $(BIN)/$(EXE) $(LIBS)

pgo :
	$(MAKE) clean
	$(CC) $(CFLAGS) $(PGO_GEN) hqperft.c -o $(BIN)/$(EXE) $(LIBS)
	cd $(BIN); LLVM_PROFILE_FILE=hqperft-%p.profraw ./$(EXE) -d 6 -b | grep perft; 
	cd $(BIN); LLVM_PROFILE_FILE=hqperft-%p.profraw ./$(EXE) -d 7 -b -H 24 | grep perft; 
	$(PGO_MERGE)
	$(CC) $(CFLAGS) $(PGO_USE) hqperft.c -o $(BIN)/$(EXE) $(LIBS)

release:
	$(MAKE) clean
	$(CC) $(CFLAGS) -DRELEASE $(PGO_GEN) hqperft.c -o $(BIN)/$(EXE) $(LIBS)
	LLVM_PROFILE_FILE=$(BIN)/hqperft-%p.profraw $(BIN)/$(EXE) < bench.scr
	$(PGO_MERGE)
	$(CC) $(CFLAGS) -DRELEASE $(PGO_USE) hqperft.c -o $(BIN)/$(EXE) $(LIBS)
